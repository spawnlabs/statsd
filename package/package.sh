...
fpm -s dir -t deb -a all -n $PACKAGENM -v $SPAWNENV_PROJECT_BUILDNO -p target/deb/${PACKAGENM}_${SPAWNENV_PROJECT_BUILDNO}_all.deb --before-remove prerm.sh --after-remove postrm.sh --before-install preinst.sh --after-install postinst.sh -C target/src/ opt/$PACKAGENM etc
